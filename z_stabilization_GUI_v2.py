...
class Frontend(QtGui.QFrame):
    ...
    def setUpGUI(self, show_piezo_subGUI):
        ...
        self.ROIbox_vertical_pos.editingFinished.connect(self.roi_edit_changed)
        self.ROIbox_horizontal_pos.editingFinished.connect(self.roi_edit_changed)
        self.ROIbox_vertical_size.editingFinished.connect(self.roi_edit_changed)
        self.ROIbox_horizontal_size.editingFinished.connect(self.roi_edit_changed)
    ...
    def roi_changed_check(self):
        vertical_pos = int(self.roi.pos()[1])
        horizontal_pos = int(self.roi.pos()[0])
        vertical_size = int(self.roi.size()[1])
        horizontal_size = int(self.roi.size()[0])
        roi_list = [vertical_pos, horizontal_pos, vertical_size, horizontal_size]
        if roi_list != self.roi_list_previous:
            self.roi_list_previous = roi_list
            print('ROI has been changed.')
...
class Backend(QtCore.QObject):
    ...
    def __init__(self, piezo_z, piezo_backend, connect_to_piezo_module = True, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.connect_to_piezo_module = connect_to_piezo_module

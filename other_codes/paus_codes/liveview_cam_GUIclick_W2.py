...
# initial fake image
initial_image_np = 128*np.ones((1440, 1080, 3))
...
class Frontend(QtGui.QFrame):
    ...
    def __init__(self, main_app = True, *args, **kwargs):
        ...
        self.setGeometry(5, 30, 900, 764) # x pos, y pos, width, height
    ...
    def setUpGUI(self):
        ...
        self.vb = self.imageWidget.addViewBox()
        self.vb.setLimits(xMin=-max_y_cursor/2, xMax=max_y_cursor/2, yMin=-max_x_cursor/2, yMax=max_x_cursor/2)
        
        self.tr = QtGui.QTransform()  # prepare ImageItem transformation:
        self.tr.rotate(-90)
        self.tr.translate(-720, -540) # move image 
        ...
        #self.vb.hideAxis('left')
        #self.vb.hideAxis('bottom')
        self.view_box = self.vb.getViewBox()
        ...
        # Exposure time V2
        exp_label = QtGui.QLabel('Exposure time (ms):')
        self.exp_time_edit = QtGui.QSlider(QtCore.Qt.Horizontal)
        self.exp_time_edit.setMinimum(0)
        self.exp_time_edit.setMaximum(200)
        self.exp_time_edit.setValue(100)
        self.exp_time_edit.setTickPosition(QtGui.QSlider.TicksBelow)
        self.exp_time_edit.setTickInterval(1)
        self.exp_time_edit.sliderReleased.connect(self.exposure_changed_check)
        self.exp_value_label = QtGui.QLabel('100')
        self.exp_value_label.setText('%d' % int(self.exp_time_edit.value()))
        ...
        self.vb.scene().sigMouseClicked.connect(self.plot_clicked)
       
        self.mov_x_sl = QtGui.QSlider(QtCore.Qt.Horizontal)
        self.mov_x_sl.setMinimum(-max_x_cursor/2)
        self.mov_x_sl.setMaximum(max_x_cursor/2)
        self.mov_x_sl.setValue(0)
        ...
        self.mov_y_sl.setMinimum(-max_y_cursor/2)
        self.mov_y_sl.setMaximum(max_y_cursor/2)
        self.mov_y_sl.setValue(0)
        ...
        cursor_x_label = QtGui.QLabel('Cursor Y')
        self.cursor_x = QtGui.QLabel('NaN')
        cursor_y_label = QtGui.QLabel('Cursor X')
        ...
        """
        # Exposure time box V2
        layout_liveview.addWidget(exp_label,              6, 0)
        layout_liveview.addWidget(self.exp_value_label,   6, 1)
        layout_liveview.addWidget(self.exp_time_edit,        7, 0, 1, 2)
        
        
        
        # Gain box
        layout_liveview.addWidget(gain_label,              8, 0)
        layout_liveview.addWidget(self.gain_value_label,   8, 1)
        layout_liveview.addWidget(self.gain_slider,        9, 0, 1, 2)
        # auto level
        layout_liveview.addWidget(self.autolevel_tickbox,      10, 0)
        # pixel size
        layout_liveview.addWidget(pixel_size_Label,      11, 0)
        layout_liveview.addWidget(self.pixel_size,       11, 1)
        ...
        viewDock = Dock('Camera', size = (200, 200/optical_format))
    ...
    def plot_clicked(self,event):
        ...
        if self.mov_x_sl.isEnabled(): #check if cursor movement is enabled
            
            self.pos = event.scenePos() # Get the coordinates of the click event
            self.mouse_point = self.vb.mapSceneToView(self.pos) #convert them to the image coordinates
            #print(self.pos)
            #print(self.mouse_point.x(),self.mouse_point.y())
            self.point_graph_cursor.setData([self.mouse_point.x()], [self.mouse_point.y()])
    

    def exposure_changed_check(self):
        self.exp_value_label.setText(format(int(self.exp_time_edit.value())))
        self.exp_time = int(self.exp_value_label.text())
        if self.live_view_button.isChecked():
            self.exposureChangedSignal.emit(True, self.exp_time)
        else:
            self.exposureChangedSignal.emit(False, self.exp_time)
        return
    ...
    def take_picture_button_check(self):
        exposure_time_ms = self.exp_time_edit.value() # in ms
        if self.live_view_button.isChecked():
    ...
    def get_cursor_values(self, data_cursor):
        ...
        print ([point_cursor_y], [point_cursor_x])

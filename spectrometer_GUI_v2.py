...
from rotator_toolbox import Rotation_mount 
...
class Frontend(QtGui.QFrame):
    ...
    rotateDiffuserSignal = pyqtSignal(bool)
    ...
    def setUpGUI(self):
        ...
        spectrum_parameters_layout.addWidget(self.rotate_diffuser_tickbox,      7, 0)
    ...
    def set_rotating_diffuser(self):
        if self.rotate_diffuser_tickbox.isChecked():
            self.rotateDiffuserSignal.emit(True)
        else:
            self.rotateDiffuserSignal.emit(False)
        return
...
class Backend(QtCore.QObject):
    ...
    def __init__(self, mySpectrometer, myCamera, myRotaMount, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.myCamera = myCamera
        self.mySpectrometer = mySpectrometer
        self.myRota = myRotaMount
        self.myRota.homing()
    ...
    @pyqtSlot(bool)
    def rotate_diffuser_action(self, rotate_bool):
        if rotate_bool:
            self.rotate_diffuser = True
            print('The diffuser will rotate during acquisition.')
        else:
            self.rotate_diffuser = False
            print('The diffuser will NOT rotate during acquisition.')
        return    
    ...
    def make_connection(self, frontend): 
        ...
        frontend.rotateDiffuserSignal.connect(self.rotate_diffuser_action)
...
if __name__ == '__main__':
    ...
    print('Connecting the rotating mount...')
    rot_mount = Rotation_mount(ROT_MOUNT_PORT)

    gui = Frontend()
    worker = Backend(mySpectrometer, camera, rot_mount)
